...
def GKTH_ksubsample(p, layers):
    ...
    if p.use_kspace_subsampling:
        ...
        def anonymous_function(x):
            return make_grid(avg_spectrum, x)[0] / (p.nkpoints ** 2) - p.subsampling_point_fraction
        ...
        try:
            ...
            best_scale = minimize_scalar(anonymous_function, bounds=(0, 100), method='bounded', options=options_bounded)
        except Exception as e:
            try:
                # Second attempt with different bounds and 'bounded' method
                best_scale = minimize_scalar(anonymous_function, bounds=(-100, 1000), method='bounded', options=options_bounded)
            except Exception as e:
                # Final fallback using 'brent' method where 'fatol' is valid
                best_scale = minimize_scalar(anonymous_function, method='brent', options=options_brent)
