...
class Layer:
    ...
    def __init__(self, n=None):
        ...
        self.tNN = -1.5     #Nearest neighbour hopping parameter in eV
        self.tNNN = -0.15    #Next-nearest neighbour hopping parameter in eV
    ...
    def xis(self, p):
        if self.dispersion_type == 'tb':
            return self.tNN * (np.cos(p.k1 * p.a) + np.cos(p.k2 * p.a)) + \
                   self.tNNN * np.cos(p.k1 * p.a) * np.cos(p.k2 * p.a) + self.mu
        elif self.dispersion_type == 'para':
            hbar = 6.582119569e-16  # Planck's constant in eV.s
            me = 9.10938356e-31    # Electron mass in kg
            return hbar**2 * ((p.k1 * p.a)**2 + (p.k2 * p.a)**2) / (2 * me) + self.mu
        else:
            raise ValueError("Spectrum type must be 'tb' for tight binding or 'para' for parabolic.")