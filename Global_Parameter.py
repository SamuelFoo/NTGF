...
class GlobalParams:
    ...
    def k1(self):
        ...
        if self.lattice_symmetry in ["mm", "4mm"]:
            ks = scale * np.linspace(
                (1 / (2 * self.nkpoints)), (1 - 1 / (2 * self.nkpoints)), self.nkpoints
            )
            k1, _ = np.meshgrid(ks, ks)
            for i in range(self.nkpoints):
                # Shift by a 10th of a step size with alternating sign to avoid divide by zeros
                k1[i, i] = k1[i, i] + (2 * (i % 2) - 1) * scale / (self.nkpoints * 10)
        else:
            ks = scale * np.linspace(
                (-1 + 1 / (self.nkpoints)), (1 - 1 / (self.nkpoints)), self.nkpoints
            )
            k1, _ = np.meshgrid(ks, ks)
            for i in range(self.nkpoints):
                # Shift by a 10th of a step size with alternating sign
                k1[i, i] = k1[i, i] + (2 * (i % 2) - 1) * scale / (self.nkpoints * 10)
                # Shift by a 5th of a step size with alternating sign
                k1[i, self.nkpoints - i - 1] = k1[i, self.nkpoints - i - 1] + (
                    2 * (i % 2) - 1
                ) * scale / (self.nkpoints * 10)
        return k1
    ...
    def k2(self):
        ...
        if self.lattice_symmetry in ["mm", "4mm"]:
            ...
            for i in range(self.nkpoints):
                # Shift by a 10th of a step size with alternating sign
                k2[i, i] = k2[i, i] - (2 * (i % 2) - 1) * scale / (self.nkpoints * 10)
        else:
            ks = scale * np.linspace(
                (-1 + 1 / (self.nkpoints)), (1 - 1 / (self.nkpoints)), self.nkpoints
            )
            ...
            for i in range(self.nkpoints):
                ...
                # Shift by a 10th of a step size with alternating sign
                k2[i, self.nkpoints - i - 1] = k2[i, self.nkpoints - i - 1] + (
                    2 * (i % 2) - 1
                ) * scale / (self.nkpoints * 10)
        return k2
